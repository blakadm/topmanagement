...
<div class="row" style="padding-left: 10px">
    <div class="col-lg-2 box-blue" style="padding-bottom: 10px">
        ...
        <?php
        echo $form->field($model, 'LabID')->widget(Select2::classname(),[
           'name' => 'LabID',
            'value' => $model->LabID,
            'size'=>'sm',
            'data' => $model->LabData,
        ...
        <?php
            ...
            'pluginEvents' => ['change' => '
                function(e){
                   var value = $(e.currentTarget).find("option:selected").val();
                   var EndValue=$("#FrequencyYear2").val();
                   if(EndValue<value){
                       EndValue=value;
                   }
                   $("#FrequencyYear2").val(EndValue).trigger("change");
                   $("#StartYear").val(value);
                }
            ']
            ]);
    ...
    <div class="col-lg-10">
        <div class="col-lg-8 box box-primary" style="height: 500px">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 box-body">
                ...
                if($model->ReferralMode==1){
                    $RMode="RL";
                }else{
                    $RMode="TCL";
                }
                $model->ChartTitle="Total Referrals-$RMode".$model->ChartTitle;
                Yii::$app->PostedData->GenerateChart("Referral",$model->ChartType, "Referrals", $model->ChartTitle, "/toplevel/statistics/referral", $Categories, $model->Series,850,400,"sand-signika","0.f");
                ?>
            </div>
        </div>
    </div>  
